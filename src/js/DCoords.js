export default class DCoords {
  ...
  static getArea(position, dist, math, boardSize) {
    ...
    const min = {
      x: Math.max(coordsO.x - dist, 0),
      y: Math.max(coordsO.y - dist, 0),
    };
    const max = {
      x: Math.min(coordsO.x + dist, boardSize - 1),
      y: Math.min(coordsO.y + dist, boardSize - 1),
    };
    for (let { x } = min; x <= max.x; x += 1) {
      for (let { y } = min; y <= max.y; y += 1) {
        const coordsItem = { x, y };
        if (this.distance(coordsItem, coordsO, math) <= dist) {
